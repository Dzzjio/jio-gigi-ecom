...
const ProfileModal = () => {
  ...
  const handleParametersClick = () => {
    closeModal(); // Close the modal when clicking on parameters
  };
  ...
  return (
    <div>
      <ModalHeader>
        <ModalTitle>
          {fullUser?.first_name ? (
            <p>გამარჯობა, {fullUser?.first_name}</p>
          ) : (
            <p>თქვენ არ ხართ დარეგისტრირებული</p>
          )}
        </ModalTitle>
      </ModalHeader>
      <ModalContent>
        <Link to='/profile' onClick={handleParametersClick}>
          <span>
